// [16.2 Option & unwrap](http://rustbyexample.com/error/option_unwrap.html)
/*
In the last example, we showed that we can induce program failure at will.
We told our program to panic if the princess received an inappropriate gift
- a snake. But what if the princess expected a gift and didn't receive one?
...
Note that it's possible to manually customize panic with [expect](http://doc.rust-lang.org/std/option/enum.Option.html#method.expect),
but unwrap otherwise leaves us with a less meaningful output than explicit handling. In the following example, explicit handling yields a more controlled result while retaining the option to panic if desired.
