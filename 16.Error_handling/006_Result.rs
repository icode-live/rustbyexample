// [16.3 Result](http://rustbyexample.com/error/result.html)
/*
[Result](http://doc.rust-lang.org/std/result/enum.Result.html)
is a richer version of the
[Option](http://doc.rust-lang.org/std/option/enum.Option.html) type
that describes possible error instead of possible absence.
...
for example, either yields the element T or panics.
For case handling, there are many combinators between Result and Option that overlap.

In working with Rust, you will likely encounter methods
that return the Result type, such as the parse() method.
...
to panic on. 
Additionally, the panic exits our program and provides an unpleasant 
error message.
