// [5.1 Literals](http://rustbyexample.com/cast/literals.html)
...
/*
 * NOTE:
bar(&foo) is used to pass an argument to a function by reference,
rather than by value (bar(foo)).

std::mem::size_of_val is a function, but called with its full path.

Code can be split in logical units called modules.
In this case, the size_of_val function is defined in the mem module,
and the mem module is defined in the std crate.
*/
